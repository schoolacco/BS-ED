...
def cash_increase():
    ...
    # Base multiplier
    multiplier_value = stat_increment["Stats"]["Multiplier"]
    ...
    if isinstance(stat_increment["Stats"]["Cash"], Mantissa):
        stat_increment["Stats"]["Cash"] += cash_increment
    else:
        try:
          stat_increment['Stats']["Cash"] += cash_increment.to_float()
          stat_increment['Stats']["Cash"] = round(stat_increment["Stats"]["Cash"], 2)
        except TypeError:
          stat_increment['Stats']["Cash"] = float_to_mantissa(stat_increment['Stats']["Cash"])
          stat_increment["Stats"]["Cash"] += cash_increment
    # Update label
    val = stat_increment["Stats"]["Cash"]
...
def gem_increase():
    global stat_increment
    stat_increment["Stats"]["Gems"] += (1+upgrades["gem_timer_amount"]["current_lvl"]*upgrades["gem_timer_amount"]["effect"])
...
def event_increase():
    global stat_increment
    stat_increment["Stats"]["Event Power"] += (1+upgrades["event_timer_amount"]["current_lvl"]*upgrades["event_timer_amount"]["effect"])
...
def reset_button(cost, unit, reward, unit_2):
    ...
    stat_list = list(abs_stat_info["Main Progression"].keys())
    ...
    if current_value >= cost:
        ...
        r_text = stat_increment["Stats"]['Rebirths'] if not isinstance(stat_increment["Stats"]['Rebirths'], Mantissa) else stat_increment['Stats']['Rebirths'].to_string()
        # Update labels
        cash_l.setText(f"Cash: {stat_increment['Stats']['Cash']}")
        multi_l.setText(f"Multiplier: {stat_increment['Stats']['Multiplier']}")
...
def reset_button_special(cost, unit, reward, unit_2, resets=[]):
    ...
    if current_value >= cost:
        ...
        if isinstance(reward_m, Mantissa):
            ...
            value += reward_m
            value = value.to_float() if value.exp < math.log(MANTISSA_THRESHOLD, 10) else value
            stat_increment["Stats"][unit_2]["Value"] = value
        m_text = stat_increment['Multiplier']['Value'] if not isinstance(stat_increment['Multiplier']['Value'], Mantissa) else stat_increment['Multiplier']['Value'].to_string()
        r_text = stat_increment['Rebirths']['Value'] if not isinstance(stat_increment['Rebirths']['Value'], Mantissa) else stat_increment['Rebirths']['Value'].to_string()
        # Update labels
        cash_l.setText(f"Cash: {stat_increment['Cash']['Value']}")
        multi_l.setText(f"Multiplier: {m_text}")
        re_l.setText(f"Rebirth: {r_text}")
        stat_increment["Stats"]["Buttons Pressed"] += 1
...
multi_l = QLabel()
re_l = QLabel()
abs_stat_info = {"Main Progression": {
     "Cash":  {"Multis": None}, 
    ...
    "Secret": {
        ...
        "Starglass": {"Multis": {"Cash": Mantissa(1,303)}},
